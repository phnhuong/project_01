# Development Setup Guide - Hệ Thống Quản Lý Học Sinh
...
│   └── Proxy: Backend API requests → localhost:3000
│
├── Backend (Port 3000)
│   ├── Node.js 20.x
│   ├── Express.js
│   ├── Managed by PM2
│   └── Path: ~/qlhs_02/backend
│
├── Frontend
│   ├── React (built to static files)
│   └── Path: ~/qlhs_02/frontend/dist
│
...
⚠️ **CRITICAL:** Password `admin123` is for initial setup only.  
...
# Application
VITE_APP_NAME=Hệ Thống Quản Lý Học Sinh
...
async function main() {
  ...
  const subjects = await Promise.all([
    prisma.subject.create({ data: { code: 'TOAN', name: 'Toán', credits: 4 } }),
    prisma.subject.create({ data: { code: 'LY', name: 'Vật Lý', credits: 3 } }),
    prisma.subject.create({ data: { code: 'HOA', name: 'Hóa Học', credits: 3 } }),
    prisma.subject.create({ data: { code: 'SINH', name: 'Sinh Học', credits: 2 } }),
    prisma.subject.create({ data: { code: 'VAN', name: 'Ngữ Văn', credits: 4 } }),
    prisma.subject.create({ data: { code: 'ANH', name: 'Tiếng Anh', credits: 3 } }),
    prisma.subject.create({ data: { code: 'SU', name: 'Lịch Sử', credits: 2 } }),
    prisma.subject.create({ data: { code: 'DIA', name: 'Địa Lý', credits: 2 } }),
    prisma.subject.create({ data: { code: 'GDCD', name: 'GDCD', credits: 1 } }),
    prisma.subject.create({ data: { code: 'TD', name: 'Thể Dục', credits: 2 } }),
    prisma.subject.create({ data: { code: 'TIN', name: 'Tin Học', credits: 2 } }),
  ]);
